...
class ObjectsContainer(object):
  ...
  def get_fw_address_obj(self, addr, verbose=True):
    ...
    if verbose:
      ...
      address += [_SP * 2 + 'set comment "%s"' % self.FixCommentLength(addr_comment)]
  ...
  def get_fw_svcgrps(self):
    ...
    for svcgrp, value in self._dict_svcgrps.items():
      svcgrps += [_SP + 'edit %s' % svcgrp]
      if self.verbose:
        svcgrps += [_SP * 2 + 'set comment "%s"' % _DEFAULT_COMMENT]
      svcgrps += [_SP * 2 + value]
      svcgrps += [_SP + 'next']
  ...
  def FixCommentLength(self, comment):
  ...
  def __str__(self):
    ...
    if self._term.comment and self._term.verbose:
      lines += [_SP * 2 + 'set comments "%s"' %
                self._obj_container.FixCommentLength((' ').join(self._term.comment))]
